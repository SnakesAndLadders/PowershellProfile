...
$status = ($returnCodeMessage.Keys | Where { ($_ -band $i) -or ($_ -eq 0x00) } | Select -First 1)
$message = $returnCodeMessage[$status]
if($status -eq 0x00 -Or $status -eq 0x01 -Or $status -eq 0x02){
	exit 
}else{
	throw $message
}
